<template>
  <ul class="nav nav-tabs mt-5" id="myTab" role="tablist">
    <li class="nav-item" role="presentation">
      <button class="nav-link active" id="animales-tab" data-bs-toggle="tab" data-bs-target="#animales" type="button" role="tab" aria-controls="animales" aria-selected="true">Animales</button>
    </li>
    <li class="nav-item" role="presentation">
      <button class="nav-link" id="insumos-tab" data-bs-toggle="tab" data-bs-target="#insumos" type="button" role="tab" aria-controls="insumos" aria-selected="false">Insumos</button>
    </li>
  </ul>
      ...
      <div class="text-center">
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-2 mt-3">
          <button class="btn btn-secondary text-white" type="button"
                  v-on:click="openAddOperationModal(null)">
            Nuevo Operacion
          </button>
        </div>
        <div class="col-12 table-responsive">
          <table class="table">
            <caption>Mostrando {{ operationsLength }} de {{ operationsLength }}
              Operaciones
            </caption>
            <caption>Ingreso: ${{ results.income }} <br>
                     Egreso:  ${{ results.expenses }}<br>
            ...
            <tbody>
            <tr v-for="(operation) in operations" :key="operation.id" :class="operation.operationType=='sell'?'sell':'buy'">
              ...
              <td class="text-end">
                <font-awesome-icon
                    class="me-2"
                    icon="edit"
                    @click.stop="openAddOperationModal(operation)">
                </font-awesome-icon>
              </td>
            </tr>
            </tbody>
...
export default {
  ...
  components: { ConfirmationModal, OperationModal },
  mounted() {
  ...
  methods: {
    ...
    async searchOperations(page=0, size=10){
      ...
      this.listOperations({page: page, size: size}).then(
        (response) => {
...
<style>
.sell{background-color: #97e089;}
.buy{background-color: #c56161;}
</style>