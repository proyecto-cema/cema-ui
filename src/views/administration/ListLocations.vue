<template>
  <div class="text-center">
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-2 mt-3">
      <button class="btn btn-secondary text-white" type="button" v-on:click="openAddLocationModal(null, null)">
        Nueva ubicación
      </button>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="(location, index) in locations" :key="location.name">
            ...
            <td>{{ location.size }}</td>
            <td class="text-end">
              <font-awesome-icon class="me-2" icon="edit" @click.stop="openAddLocationModal(index, location)">
              </font-awesome-icon>
              <font-awesome-icon icon="trash" @click.stop="formDeleteLocation(index, location.name)">
              </font-awesome-icon>
            </td>
          </tr>
        </tbody>
  ...
  <location-modal
    ...
    @deleteModal="deleteLocationForm"
    @createdNew="addLocationToList"
  ></location-modal>
</template>
<script>
