<template>
  <div class="text-center">
    ...
    <div class="col-12 table-responsive">
      <table class="table">
        <caption>Mostrando {{ establishmentsLength }} de {{ establishmentsLength }}
          establecimientos
        </caption>
        <thead>
        ...
        <tr v-for="(establishment, index) in establishments" :key="establishment.cuig">
          ...
          <td class="text-end">
            <font-awesome-icon
                class="me-2"
                icon="edit"
                @click.stop="openAddEstablishmentModal(establishment)">
            </font-awesome-icon>
            <font-awesome-icon
                icon="trash"
                @click.stop="formDeleteEstablishment(index, establishment.cuig)">
            </font-awesome-icon>
          </td>
        </tr>
        </tbody>
  ...
  <confirmation-modal
      :confirmation-message="'¿Confirma que desea eliminar el establecimiento con CUIG '
      + deleted['cuig'] + '?'"
  ...
  <establishment-modal modalId="addEstablishmentModal" @deleteModal="deleteEstablishmentForm" @createdNew="addEstablishmentToList"></establishment-modal>
</template>
<script>
