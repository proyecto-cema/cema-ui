<template>
  <div class="row">
    <active-bovines-card
      v-for="chartHelper in activeBovines"
      :active-quantity="chartHelper.activeQuantity"
      :alt-display="chartHelper.altDisplay"
      :image-display="chartHelper.imageDisplay"
      :key="chartHelper.altDisplay"
    ></active-bovines-card>
  </div>
  <div id="chartCarousel" class="carousel carousel-dark slide" data-bs-ride="carousel" data-bs-interval="false">
    <div class="carousel-inner">
      <div
        class="carousel-item"
        :class="chartActive === chartHelper.endpoint ? 'active' : ''"
        v-for="chartHelper in chartIterator"
        :key="chartHelper.endpoint"
      >
        <cema-data-chart
          class="container"
          :chart-type="chartHelper.chartTypes"
          :title="chartHelper.title"
          :dates="chartHelper.dates"
          :endpoint="chartHelper.endpoint"
          :chart-id="chartHelper.endpoint + '-chart'"
          :ref="chartHelper.endpoint"
        >
        </cema-data-chart>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    this.retrieveReportData({ name: 'live', yearsTo: thisYear, decrement: 2000, arrangeData: false }).then(
      (reportedList) => {
