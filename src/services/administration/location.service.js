...
class LocationService {
    setLocation (location, edit) {
        if (edit) {
            return httpActivity.put('locations/' + encodeURIComponent(location.previousName), location,
    ...
    getLocation (location, cuig) {
        return httpActivity.get('locations/'+ encodeURIComponent(location.name),
            { params:{cuig: cuig}, headers: authHeader() }
        )
    }

    deleteLocation (location, cuig) {
        return httpActivity.delete('locations/'+ encodeURIComponent(location.name),
            { params:{cuig: cuig}, headers: authHeader() }
        )
    }
